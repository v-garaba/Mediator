    ...
    : INotificationHandler<TrackMessageNotification>
{
    ...
    private readonly IStorage<UserRef, UserNotification> _userNotificationStorage = userNotificationStorage.AssertNotNull();

    public async Task HandleAsync(TrackMessageNotification notification)
    {
        notification.AssertNotNull();
        var userNotification = await _userNotificationStorage.TryGetAsync(notification.UserId)
            ?? new UserNotification(notification.UserId, 0);

        await _userNotificationStorage.SetAsync(
            userNotification with { MessageCount = userNotification.MessageCount + 1 }
        );
